...
const EMPTY_RESULT: SearchResult = { students: [], employees: [], guardians: [] };

// Phase 8 "global search" bullet, part 1 — people only (students,
// staff, guardians). Admin /dashboard only; same click-outside
// dismissal pattern as NotificationBell. Results link to their
// entity's existing list page with a `highlight` query param — there
// is no per-record detail route in this app yet, so "jump to and
// scroll to the right row" is the honest, buildable destination (see
// the `highlight` handling added to the Students/Staff pages).
export function GlobalSearchBox() {
  ...
  const hasResults = result.students.length > 0 || result.employees.length > 0 || result.guardians.length > 0;
  ...
  return (
    <div className="relative w-64" ref={containerRef}>
      ...
      <Input
        className="pl-8"
        placeholder="Search students, staff, guardians…"
        value={query}
