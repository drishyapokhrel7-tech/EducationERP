...
type Result = { state: "success" } | { state: "failed"; message: string };
...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    api
      .confirmPortalEsewaPayment(data)
    ...
    <Card className="max-w-md">
      ...
      <CardContent className="space-y-4">
        ...
        {outcome.state === "success" ? (
          <p className="text-sm text-emerald-700">Payment received — your invoice has been updated.</p>
        ) : null}
        {outcome.state === "failed" ? <p className="text-destructive text-sm">{outcome.message}</p> : null}
        <Link href="/portal/invoices" className={buttonVariants({ size: "sm" })}>
          Back to My Invoices
        </Link>
      </CardContent>
    </Card>
...
export default function PortalEsewaCallbackPage() {
  return (
