    ...
    <Card className="max-w-md">
      ...
      <CardContent className="space-y-4">
        ...
        {outcome.state === "success" ? (
          <p className="text-sm text-emerald-700">Payment received — the invoice has been updated.</p>
        ) : null}
        {outcome.state === "failed" ? <p className="text-destructive text-sm">{outcome.message}</p> : null}
        <Link href="/dashboard/finance" className={buttonVariants({ size: "sm" })}>
          Back to Finance
        </Link>
      </CardContent>
    </Card>
