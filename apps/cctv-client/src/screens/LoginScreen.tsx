...
export function LoginScreen({ onLoggedIn }: { onLoggedIn: (user: SafeUser) => void }) {
  ...
  // login surface in this project — this is a one-time interactive
  // sign-in (the station then stays signed in via its own refresh
  // token), still done by a real person once, not exempt.
  ...
  async function loadCaptcha() {
    setCaptchaAnswer("");
    const challenge = await window.cctvClient.getCaptcha();
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const user = await window.cctvClient.login({ identifier, password, captchaId, captchaAnswer });
      onLoggedIn(user);
    } catch {
